...
That matters for this conversation because it sets the size of the question. The only asset ADT.ai holds is a database credential, and the only privileges it can use are the ones you grant its user.
...
- **The key does not live with the file.** It comes from `-key` or the `ADT_KEY` environment variable, and either may be a value or **a path to a key file of any length, stored anywhere on the machine**. So the encrypted connection file and the key that opens it need never travel together, which is what makes a leak of one of them survivable.
...
**Encryption in the connection file is not a substitute for your own controls.** It raises the cost of a leaked file. It does not limit what the credential can do once someone has it. That limit is set by the privileges you grant, and it is the subject of the next section.
