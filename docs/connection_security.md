...
- A connection file reached through a symbolic link is written at the link's target, by `adtai connection` and by SQLcl registration alike, and the link stays a link. A project that links its `connections.yaml` to a file kept elsewhere therefore never receives a regular copy of that file, password included.
...
- **Connection fields cannot introduce another SQLcl command.** A username, hostname, service, saved connection name, or external authentication alias containing a line break is refused before SQLcl starts. The error names the field, without printing its value. Ordinary quoted usernames and punctuation remain supported.
- **A connection file that does not parse is refused without its contents.** A YAML parser's own message quotes the offending line, and in a connection file that line can be the `pwd:` one; a repeated key is quoted with both of its values. The refusal names the file, the line and the column, and nothing the file holds, in a `-debug` traceback too.
- **Temporary scripts are locked down and removed.** A script that does carry a password, such as the registration run, is written to the operating system's temporary folder rather than the project, so a synced project folder never receives it. It is written with `0600` permissions, readable only by the owner, and deleted after the run; one that a killed run leaves behind is removed the next time ADT.ai writes a SQLcl script (on Windows, once it is ten minutes old). Any password in captured SQLcl output is replaced with `***` before that output reaches a log or the screen. Longer secrets are masked first, so a password that contains another password cannot leave its remaining characters exposed.
