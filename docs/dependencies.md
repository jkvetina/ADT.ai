...
On a first refresh this prerequisite is the slowest thing the command does, so it crawls rather than holding the section header still: one redrawn `RECOMPILING DUE TO WRONG PL/SCOPE` row, opened before the session `ALTER` so the whole-schema scan behind it is covered too, and closed at 100%. It is one row, not a row per object, and there is no section header of its own. A refresh driven with no console reporter draws no bar at all, a progress row not being required chrome.
