...
The file is at version 3. A version 1 file, which keyed its watermarks by a TEXT application id and allowed an empty version value, is lifted in place on open with every watermark kept; a watermark row that never named an application is dropped.

A version 2 file gains `base_commit` and `mirror_ref` as added columns, so every checksum it already recorded is left exactly where it was. A file two versions behind takes both steps in one open.
