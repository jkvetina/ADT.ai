...
- **`email` and `apex_account` are the commit identity.** Every `-my` and `-by` filtering git history reads `email`: [`patch`](patch.md), [`search`](search.md), [`rebuild`](rebuild.md) and [`calendar`](calendar.md). [`export_apex -my`](export_apex.md) reads both, matching APEX workspace developers on the login as well as on the address.
...
Every section that lists files, on `patch`, `export_db` and `search` alike, opens on the folder its rows share and gives every directory below it a row of its own, each two spaces further in:
