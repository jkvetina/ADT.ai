...
- A scan the database refuses closes its row on `FAILED` and the run carries on. The application is named under `WARNING - COMPONENT SCAN FAILED, DEPENDENCIES KEPT:` as `APP 100/ORDERS kept its previous dependencies: ORA-01086: savepoint 'START_SCAN' never established in this session or is invalid`, its component source and page links are refreshed as usual, every other application still scans, and the run exits `1`.
- The scan clears the cache the `APEX_USED_DB*` views read from, so after it fails those views answer nothing and are not read at all. The application keeps the rows its last good scan wrote, and `search -impact` answers from them until a scan succeeds again.
