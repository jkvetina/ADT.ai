...
![Version](https://img.shields.io/badge/version-1.2.0-blue) ![License](https://img.shields.io/badge/license-MIT-green) ![Python](https://img.shields.io/badge/python-3.14%2B-blue) [![CI](https://github.com/jkvetina/ADT.ai/actions/workflows/ci.yml/badge.svg)](https://github.com/jkvetina/ADT.ai/actions/workflows/ci.yml)

Get your Oracle database and APEX applications into Git, and get your changes back out to the next environment.

![Database in, Git-friendly files out](https://raw.githubusercontent.com/jkvetina/ADT.ai/v1.2.0/docs/images/cover.png)
...
Every command below links to its own page in [docs/](https://github.com/jkvetina/ADT.ai/blob/v1.2.0/docs/README.md), and each description is the same text its help screen opens with.

<br>

### Export

![Get it into the repo](https://raw.githubusercontent.com/jkvetina/ADT.ai/v1.2.0/docs/images/export.png)
...
- [`export_db`](https://github.com/jkvetina/ADT.ai/blob/v1.2.0/docs/export_db.md): Brings database objects out of the database and into your repository. Use it to keep the repository the authoritative copy of the schema, so a change made anywhere shows up in a review and can be deployed again rather than reconstructed from memory. You can take a whole schema or only what has moved recently, and what comes out is written to be read by a person, not just replayed by a tool.
- [`export_data`](https://github.com/jkvetina/ADT.ai/blob/v1.2.0/docs/export_data.md): Brings the contents of tables out of the database and into your repository. Use it for the rows an application needs in order to work at all (lookup values, defaults, configuration held as data), which belong under review beside the code that reads them. Each table comes back both as something you can read in version control and as something that can put the same rows into another environment.
- [`export_apex`](https://github.com/jkvetina/ADT.ai/blob/v1.2.0/docs/export_apex.md): Brings APEX applications out of the builder and into your repository. Use it so that what was built by clicking has a history, can be reviewed like any other change, and can be deployed again somewhere else, with the application living beside the database code it runs on. It can show you what is there before you commit to exporting it, and write the result in whichever form your reviewers and your deployment need.

<br>

### Check

![Prove it still works](https://raw.githubusercontent.com/jkvetina/ADT.ai/v1.2.0/docs/images/check.png)
...
- [`doctor`](https://github.com/jkvetina/ADT.ai/blob/v1.2.0/docs/doctor.md): Checks whether this machine is set up to run ADT.ai, and fixes it when not. Use it first when something does not work: it tells a local setup problem apart from a database or repository one, which is usually the whole question. It also installs and updates what ADT.ai depends on, and sets up a new project so you are not assembling one by hand.
- [`connection`](https://github.com/jkvetina/ADT.ai/blob/v1.2.0/docs/connection.md): Manages the environments, schemas and passwords ADT.ai connects with. Use it instead of editing the connection file by hand: it knows the shape that file has to keep, so a change to one environment cannot quietly break another, and it shows you what it is about to do before it does it. Passwords are asked for when they are needed rather than typed on the command line, so they stay out of your shell history and off your screen.

<br>

## Documentation

![No problem for the AI, humans don't read manuals anyway](https://raw.githubusercontent.com/jkvetina/ADT.ai/v1.2.0/docs/images/docs.png)

- [SETUP.md](https://github.com/jkvetina/ADT.ai/blob/v1.2.0/SETUP.md) covers install and environment setup.
- [docs/why.md](https://github.com/jkvetina/ADT.ai/blob/v1.2.0/docs/why.md) makes the case for adopting it: what you answer by hand today, what each command group does instead, and a fifteen-minute trial against a development schema.
- [docs/README.md](https://github.com/jkvetina/ADT.ai/blob/v1.2.0/docs/README.md) is the command index; every command in the Commands section above links straight to its own page.
- [docs/console.md](https://github.com/jkvetina/ADT.ai/blob/v1.2.0/docs/console.md#shared-arguments) documents the flags every command shares, once, so no command page repeats them.

<br>

### Skills

![Real tools, real flags, first try](https://raw.githubusercontent.com/jkvetina/ADT.ai/v1.2.0/docs/images/skills.png)

If you work with an AI agent (Claude Code, Codex, Copilot, Cursor and friends), expose [skills/adt/SKILL.md](https://github.com/jkvetina/ADT.ai/blob/v1.2.0/skills/adt/SKILL.md): its lean command router sends the agent to only the relevant page in `docs/`, with the operational and safety boundaries needed to run the command correctly. [skills/adt-setup/SKILL.md](https://github.com/jkvetina/ADT.ai/blob/v1.2.0/skills/adt-setup/SKILL.md) handles machine setup, and [skills/README.md](https://github.com/jkvetina/ADT.ai/blob/v1.2.0/skills/README.md) explains when to use each one.

<br>

### Install

![One command, then it runs](https://raw.githubusercontent.com/jkvetina/ADT.ai/v1.2.0/docs/images/install.png)
...
1) [Export database objects](https://github.com/jkvetina/ADT.ai/blob/v1.2.0/docs/export_db.md)
2) [Export table data](https://github.com/jkvetina/ADT.ai/blob/v1.2.0/docs/export_data.md)
3) [Export APEX applications](https://github.com/jkvetina/ADT.ai/blob/v1.2.0/docs/export_apex.md)
4) [Create a patch and deploy it](https://github.com/jkvetina/ADT.ai/blob/v1.2.0/docs/patch.md)

If an AI agent runs your machine setup, point it at [skills/adt-setup/SKILL.md](https://github.com/jkvetina/ADT.ai/blob/v1.2.0/skills/adt-setup/SKILL.md) and let it walk through the whole thing itself.

<br>

### Quick Start

![TLDR, nobody reads the manual](https://raw.githubusercontent.com/jkvetina/ADT.ai/v1.2.0/docs/images/quickstart.png)
...
![A thousand ideas, nothing published](https://raw.githubusercontent.com/jkvetina/ADT.ai/v1.2.0/docs/images/articles.png)
...
All the changes are described in the ADT.ai `1.2.0` [CHANGELOG.md](https://github.com/jkvetina/ADT.ai/releases/tag/v1.2.0).
