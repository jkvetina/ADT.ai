...
views, synonyms, disabled objects, jobs), ``trailing`` (the -trailing
whitespace pass), and ``vpd`` (the -vpd policy report). Callers import
everything from ``adt_ai.recompile.queries`` and cannot tell which topic a
name comes from, which is what lets the split change without touching a
single import site.
...
from adt_ai.recompile.queries.vpd import *  # noqa: F401,F403
