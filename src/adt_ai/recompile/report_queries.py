...
WHERE 1 = 1
    ...
    AND (t.constraint_name LIKE :object_name ESCAPE '\\' OR :object_name IS NULL)
UNION ALL
SELECT
    t.owner             AS owner,
...
WHERE 1 = 1
    ...
    AND (t.index_name      LIKE :object_name ESCAPE '\\' OR :object_name IS NULL)
UNION ALL
SELECT
    t.owner             AS owner,
...
WHERE 1 = 1
    ...
    AND t.owner            = SYS_CONTEXT('USERENV', 'CURRENT_SCHEMA')
    AND t.status           = 'DISABLED'
    AND (t.trigger_name    LIKE :object_name ESCAPE '\\' OR :object_name IS NULL)
...
WHERE 1 = 1
    ...
    AND (t.job_name     LIKE :object_name ESCAPE '\\' OR :object_name IS NULL)
GROUP BY
    t.owner,
    t.job_name,
    t.status,
