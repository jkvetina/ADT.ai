...
`tests/contracts/test_package_layout.py` holds the rule so the next renderer
cannot land back inside a command. `export_db/table.py` and
`export_db/render.py` re-export the whole surface, so every existing import path
still resolves.
...
# `print_adt_pipes` stood in the old home until ADT #506. It was the batch half
# of the `TYPE | NAME` object listing and the third spelling of a shape with only
# ever one meaning, so it moved to `shared/object_list.py` beside the other two.
# Two details went with it, both of them the fork rather than the feature: it
# wrote the type cell as `"  " + 18` where the streamed renderer wrote `20`, the
# same column reached two ways, and it left the name unpadded so a batch listing
# and a streamed one could not be compared byte for byte. Deleted rather than
# kept as a re-export, because `tests/contracts/shared_readers.txt` now fails on
# a second reader of that rule and a re-export is what a next author would copy.
