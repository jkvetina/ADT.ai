...
**`-compact` replaces both summary tables with the run's own score, one row under a `RESULTS:` heading** (`#575`, Jan's shape: *"instead of printing (any) SUMMARY table, you will print just the overall score"*). Five columns, `PACKAGES` / `LINES` / `TIMER` / `COVERAGE` / `STATUS`, and the first four are the `SUMMARY PER MODULE:` total row's own cells through the same builder, so the short form and the long one cannot report two different figures for one run; they print with `ut_module` blanked too, a total over every suite needing no convention to name it. **`STATUS` is the exit code in words**, `PASS` or `ERROR`, so it reads `ERROR` on a failed or errored test, on a zero-test run, and on a package under a `-gate` threshold, and a green row above a non-zero exit is not a state the command can reach. It collapses the report and nothing else: `ERRORS & FAILURES:` above the row and `COVERAGE BELOW <n>:` below it are untouched, the same line `-silent` draws, and `RESULTS:` leads the coverage read exactly as `SUMMARY PER SUITE:` does. The polarity is `export_db -compact`'s, rows by default and the flag is the collapse, and it composes with the modes rather than outranking them, `-silent -compact` being chrome plus one row and `-verbose -compact` keeping the per-test listing above it; the one thing it does take from `-verbose` is `COVERAGE CHANGED SINCE LAST RUN:`, per-package detail inside the region the row replaces.
