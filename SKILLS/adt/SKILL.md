...
adtai ut3 -env DEV -coverage
adtai ut3 -env DEV -coverage -name ICT_COM%
```

A package is run only when **both** are true: its name matches config `ut_pattern` (default `'_UT$'`), and utPLSQL has parsed it as a `%suite` with at least one `%test`. `ut_pattern` is the selection contract, so production code can never be swept in; `-name` takes repeatable Oracle `LIKE` patterns (`%`, `_`, `\` to escape) and narrows, never widens — it selects **the suites to run in every mode**, and under `-coverage` the same patterns also select the packages the report lists. No `-name` means everything. `-schema A B` tests several schemas as separate console segments.
...
**The naming convention is four config values, not flags.** All are **Oracle** regular expressions, matched case-insensitively, and Oracle evaluates them: `REGEXP_LIKE` selects the test packages inside the dictionary query — where the old `LIKE` sat, so a schema of thousands of packages is never fetched to find a handful of suites — and `REGEXP_SUBSTR` extracts the capture groups in the same pass. `ut_owner` is the only one that defaults empty. `ut_pattern` (`'_UT$'`) selects test packages. `ut_match` (`'^(.+)_UT$'`) pairs one back to the package it tests through capture group 1 — the pairing the `-coverage` verdict columns are built on — so a project whose suites are `TEST_ABC` sets `'^TEST_'` and `'^TEST_(.+)$'` and everything follows. `ut_owner` names the schema holding the suites when they do not live beside the code; it scopes discovery, the annotation cache, `-refresh` and the `ut.run` path, while coverage is still measured in the schema under test. `ut_module` names the module a **package** belongs to (`'^[^_]+_([^_]+)'` reports both `ICT_SEC_SECURITY_UT` and `ICT_SEC_SECURITY` under `SEC`): a plain run then prints a `MODULES:` table under `SUMMARY:` — `MODULE NAME` / `PACKAGES` / the same verdict and `TIMER` columns — and `-coverage` groups its own `SUMMARY:` by module with `MODULE NAME` first, both ending on a total row whose module name is blank. A group the expression could not name reads `?`, never blank, so it is not read as a second total. It ships set, so those tables print by default; `ut_module: ''` is how a project without a module convention turns them off. **Anchor it to nothing that has to follow the module token.** Under `-coverage` it is matched against each listed package's own name first and falls back to its suite's only where that finds nothing, so an expression ending `_UT$` can never read a package that has no suite — and because `-name` selects the listing and the suites independently, a filtered run then drops most of its packages into the `?` row. A trailing `_` is the same mistake one token further in: it cannot read `ICT_VPD`, a module whose whole implementation is one package, which is what put one package of Jan's 58 in that row.

`-coverage` **replaces the run report with a coverage report.** The suites still execute — running the code is how Oracle collects block coverage — but quietly: `UNIT TESTS SUITES:` and `TEST RESULTS:` are suppressed, and what prints is two tables and a roll-up. The `CODE COVERAGE:` header lands **before** the suites start, because the run is silent and a schema that takes tens of seconds over it would otherwise sit on the connection block the whole time, reading as a hang. `CODE COVERAGE:` is `PACKAGE` / `LINES` / `PASSED` / `FAILED` / `ERRORED` / `COVERAGE`, percentage right-aligned and rendered to one decimal place with no `%` (`88.0`, `41.9`) so the figures stack under each other; `NO CODE COVERAGE:` is `PACKAGE` / `LINES` **only**, because the split is on whether anything executed the package and every other column would read the same on every row of that half. `ERRORS & FAILURES:` survives, on the same condition as always — a `FAILED` column whose message needs a second run is not a report — so a green run is the two tables and nothing else. **Every package is in one table or the other, including the ones no test covers**, because a report built from coverage data alone can only describe packages that were executed and would silently omit the one you are looking for; test packages are the sole exclusion, by `ut_pattern`, since they pair 1:1 with the packages under test. `LINES` is the listed package's own `PACKAGE BODY` row count from `ALL_SOURCE` — never its test partner's, never the spec — and it is what makes the second table a priority order rather than an alphabet. `SUMMARY:` closes the section with **one row for the whole schema** — `PACKAGES` / `LINES` / `COVERED` / `COVERAGE`, counted across both tables. `COVERED` is source lines that actually ran: the distinct `dbmspcc_blocks.line` values carrying a covered block, out of the map the report already joins rather than a second `DBMS_PROFILER` trip whose tables nothing here has measured. The three do **not** divide into each other, deliberately — `LINES` counts every body row including the comments and declarations Oracle never instruments, `COVERED` counts instrumented lines that executed, `COVERAGE` stays covered blocks over measured blocks. `COVERAGE` blanks when nothing was measured; `COVERED` still reads `0`, since nothing executed is a measured zero. **`-name` narrows the run here exactly as it does without `-coverage`** (Jan, `#231`); the accepted cost is that a package reached only by an excluded suite reads lower than the truth, so the schema's real numbers come from `-coverage` with no `-name`. The three verdicts come from the package's test partner, paired by `ut_match`, so a package another suite covered shows a real percentage and blank verdicts; there is no `TESTS` column, since they sum to it. `PLSQL_OPTIMIZE_LEVEL` is **not** a prerequisite: level 2 is Oracle's default and block coverage is collected there anyway. Collection is utPLSQL's own — `DBMS_PROFILER` and `DBMS_PLSQL_CODE_COVERAGE` in parallel on 12.2+, never hand-rolled — and the percentage is the block figure with `COVERAGE`-pragma `NOT_FEASIBLE` blocks subtracted from the denominator, which makes it read slightly higher than utPLSQL's own HTML report, deliberately.
