...
- **Naming.** `ADT_` + the connection-file basename — `ADT_CORE23` for `connections/CORE23.yaml`. When the file defines more than one environment the name gains `_<ENV>`, and when that environment defines more than one schema it gains `_<SCHEMA>` (e.g. `ADT_APEXDEV_JANK_DEV_DA`), so every environment/schema pair maps to its own name. A project using the generic `connections.yaml` filename is named after its project folder instead.
