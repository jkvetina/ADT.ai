...
Validate one application by id — resolved offline through `config/apex_apps.yaml`, with no database round-trip:
...
- `-app ID` — resolved offline: `config/apex_apps.yaml` gives the owner and alias, which locate `apex/<owner>/<id>_<alias>/apexlang/` under the configured `path_apex`. An app with no export on disk produces a `NOTES:` row naming the path where one was expected, never a traceback.
...
| `-app`, `--app` | Yes | none | Application id(s) whose exported `apexlang/` folder to validate, resolved offline through `config/apex_apps.yaml`. |
